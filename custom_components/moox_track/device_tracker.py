...
class moox_trackScanner:
    ...
    async def _async_update(self, now=None):
        ...
        if not self.connected:
            _LOGGER.debug("Testing connection to moox_track")
            await self._api.test_connection()
            self.connected = self._api.connected
            if self.connected:
                _LOGGER.info("Connection to moox_track restored")
            else:
                return
    ...
    async def import_device_data(self):
        ...
        for device_unique_id in self._api.device_info:
            ...
            if device_info.get("moox_track_id") is not None:
                attr[ATTR_moox_track_ID] = device_info["moox_track_id"]
                for dev in self._api.devices:
                    if dev["id"] == device_info["moox_track_id"]:
                        device = dev
                        break
